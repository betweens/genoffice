...
/**
 * Files the running AI Office shell has open, from the registry it publishes
...
export function assertNotOpenInGui(abs: string, env: NodeJS.ProcessEnv): void {
  ...
  throw new CliError(
    EXIT.file,
    `AI Office has this file open: ${abs}`,
    { gui_pid: open.pid },
    {
      reason: 'file_open_in_gui',
      suggestion:
        'close the tab in AI Office first, or pass --force to write anyway (the editor may overwrite your change on its next save)',
    },
  )
}
