...
export interface DocToolDeps {
  ...
  background?: boolean
  /** open a file in the AI Office UI; wired in M4 (optional in tests) */
...
export function createDocumentTools(deps: DocToolDeps, host: SessionHost): McpToolDefinition[] {
  return [
    ...
    },
    {
      ...
      description: 'Open an existing file in the running AI Office app, focusing its tab.',
      inputSchema: {
    ...
    },
    {
      name: 'get_app_info',
      description:
        'Report AI Office version, the default save folder, the document formats this server can ' +
